...
const shortenUrlForUser = async (req, res) => {
  ...
  try {
    ...
    if (customDomain) {
      const domainRecord = await CustomDomain.findOne({ domain: customDomain, user: userId, verified: true });
      if (!domainRecord) {
        return res.status(400).json({ error: "Invalid or unverified custom domain." });
      }
    ...
    if (customId) {
      ...
      if (existingShortId && existingShortId.customDomain?.toString() !== selectedDomain?.toString()) {
        return res.status(400).json({ error: "Short ID already used in a different domain." });
      }
    }
    ...
    const existingShortId = await Url.findOne({ shortId, customDomain: { $ne: selectedDomain } });
    if (existingShortId) {
      return res.status(400).json({ error: "Short ID already used in a different domain." });
    }
