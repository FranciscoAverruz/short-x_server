...
async function getUrlStats(req, res) {
    ...
    try {
        const url = await Url.findOne({ shortId, user: userId }).populate("customDomain");
        ...
        const clicks = await Click.aggregate([
            ...
            { 
                $group: { 
                    _id: { 
                        country: { $ifNull: [{ $arrayElemAt: [{ $split: ["$location", ", "] }, 1] }, "Unknown"] },
                        device: "$deviceType" 
                    },
                    count: { $sum: 1 }
                } 
        ...
        response = clicks.reduce((acc, click) => {
            ...
            // clicks by country
            acc.locationsCount[country] = (acc.locationsCount[country] || 0) + click.count;
