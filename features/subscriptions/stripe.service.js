...
const { STPRICE_PRO_MONTHLY, STPRICE_PRO_ANNUAL, STPRICE_PREMIUM_MONTHLY, STPRICE_PREMIUM_ANNUAL, FRONTEND_URL } = require("../../config/env");
const mongoose = require('mongoose');

// function to create or updates user's subscription ********************************
const handleSubscription = async (email, plan, stripeCustomerId, stripeSubscriptionId, sessionToSend, renewalDate) => {
...
// creates suscripcion **************************************************************
const createStripeSubscription = async (userId, plan, suscriptionId, sessionToSend, renewalDate) => {
  ...
  try {
    ...
    switch (plan) {
      ...
      default: throw new Error('Plan no válido');
    }
    ...
    const newSubscription = await handleSubscription(user.email, plan, stripeCustomerId, subscription.id, { ...sessionToSend }, subscription.current_period_end * 1000);
    ...
    console.error("Error en createStripeSubscription:", error);
...
const updateStripeSubscription = async (userId, newPlan) => {
  ...
  if (newPlan === 'free_monthly' || newPlan === 'free_annual') {
    if (subscriptionBD.status === 'pending' || subscriptionBD.status === 'pendingToFree') {
      await suspendStripeCancellation(userId);
      return { success: true, message: "Cancelación suspendida. No se realizó ninguna acción." };
    }
  ...
  const subscription = await stripe.subscriptions.retrieve(subscriptionBD.stripeSubscriptionId);
...
const cancelStripeSubscription = async (userId, newPlan) => {
  console.log("newPlan en Stripe.Service >>>", newPlan)
  try {

    const user = await User.findById(userId).populate("subscription");
    if (!user || !user.subscription) throw new Error("Usuario o suscripción no encontrados");
    ...
    if (newPlan === 'free_monthly' || newPlan === 'free_annual') {
      ...
      return { success: true, message: "El cambio a un plan gratuito se completará al final del período de facturación." };
    }
    ...
    return { success: true, message: "La suscripción se cancelará al final del período de facturación." };
  } catch (error) {
    throw new Error(`Error al programar la cancelación: ${error.message}`);
  }
};
...
const suspendStripeCancellation = async (userId) => {
  ...
  try {
    ...
    if (!subscription.stripeSubscriptionId || !subscription.status || subscription.status !== "pending") {
      throw new Error("No hay cancelación pendiente o la suscripción no está activa.");
    }
    ...
    await Subscription.findByIdAndUpdate(subscription._id, {
      ...
      renewalDate: new Date(updatedStripeSubscription.current_period_end * 1000),
    });

    return { success: true, message: "La cancelación de la suscripción ha sido suspendida." };
  } catch (error) {
    throw new Error(`Error al suspender la cancelación de la suscripción: ${error.message}`);
  }
};
...
const getUserPaymentHistory = async (stripeCustomerId, limit = 10) => {
  try {
    ...
    console.error('Error al obtener el historial de pagos desde Stripe:', error);
